...
export class NostrEmitter {
  ...
  async connect(address, secret) {
    ...
    if (address !== undefined || this.socket.readyState > 1) {
      ...
      this.pubkey = await nobleSecp256k1.schnorr.getPublicKey(this.privkey, true)
  ...
  async eventHandler(data) {
    ...
    if (!nobleSecp256k1.schnorr.verify(sig, id, pubkey)) {
  ...
  async getSignedEvent(event) {
    ...
    event.sig = await nobleSecp256k1.schnorr.sign(event.id, this.privkey)

    // Verify that the signature is valid.
    if (!nobleSecp256k1.schnorr.verify(event.sig, event.id, event.pubkey)) {
      throw 'event signature failed verification!'
    }
